...
load('Tags_OPCUA.mat');
model_name = "FullFMS_2026_02_25_V0_4_10";
...
for i = 1:numel(ip)
    try
        % 1. Conecta ao servidor via SFTP
        s = sftp(ip(i), user, 'PrivateKeyFile', key);
        
        % 2. Entra na pasta remota específica da estação
        remoteDir = "/root/ProjetoGemeo/AAS/" + station(i);
        cd(s, remoteDir);
        
        % 3. Baixa o Variables.csv diretamente para a pasta atual do MATLAB
        mget(s, 'Variables.csv', localDir);
        close(s);
        
        % 4. Renomeia de Variables.csv para o nome local (ex: Variables_Master.csv)
        fileOriginal = fullfile(localDir, 'Variables.csv');
        fileDestino  = fullfile(localDir, CSVLocal(i));
        
        if exist(fileOriginal, 'file')
            movefile(fileOriginal, fileDestino, 'f');
        end
        
        fprintf('Download concluído [%s]!\n', station(i));
    catch ME
        fprintf('Erro na estação [%s] (IP: %s): %s\n', station(i), ip(i), ME.message);
    end
end
